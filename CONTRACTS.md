...
_Rôle unique par compte pour le MVP — pas de table many-to-many._
...
_Pas d'entité `Organisation` pour le MVP — un seul organisateur pilote, `organisateur_id` directement sur Événement. À réintroduire seulement si plusieurs organisateurs tiers arrivent, après validation équipe._
...
_Une transaction peut couvrir plusieurs Billet et/ou CommandeVendeur (relation 1-N). Aucun champ spécifique à un prestataire donné._
...
_Contrainte unique (billet_id, event_id) — un vote par billet scanné et par événement. Pas de pondération pour le MVP._
...
### 1bis. Lister les événements publiés

`GET /api/events?statut=publie`
Sortie 200 : `[{ id, titre, description, statut, lieu_nom, adresse, ville, date_debut, date_fin }]`
Le statut `publie` est appliqué par défaut. Les autres statuts sont réservés aux usages internes autorisés.
Erreurs : 400 (statut invalide)
...
_Calcul en lecture seule, agrégé à la demande (pas de table dédiée pour le MVP — une requête agrégée sur Transaction/Billet/CommandeVendeur suffit à ce volume)._
...
_Le paramètre `scenario` n'existe que dans le mock — il disparaît naturellement à la bascule._
...
|---|---|
| transaction*id | UUID |
| reference_externe | VARCHAR(64), généré à l'initiate (`pg_ref*${crypto.randomUUID()}`) |
| status | string |
| scenario | string |
| created_at | datetime |
...
_Détail complet des maquettes disponible dans l'historique de cadrage — ce fichier ne reprend que la correspondance écran ↔ endpoint pour éviter les divergences d'intégration._
